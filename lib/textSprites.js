...
proto.bind = function(model, view, projection) {
  ...
  this.shader.uniforms.textScale = 0.5 * this.textScale / this.gl.drawingBufferHeight
...
function createTextSprites(gl, extents, spacing, font, size, padding, labels) {
  ...
  function addItem(t, text) {
    ...
    var w = dims.width + 2*padding
    var h = 1.5*size + 2*padding
    var location = bin.pack({ width: w, 
                              height: h })
    if(!location) {
      endTexture()
      beginTexture()
      location = bin.pack({ width: w, 
                            height: h })
    }

    //Render the text into the canvas
    context.fillText(text, location.x+w/2, location.y+h/2)

    //Calculate vertex data
    var textureId = textures.length
    var coord = [ (location.x+padding)/canvas.width, (location.y+padding)/canvas.height ]
    var shape = [ (w-2*padding)/canvas.width, (h-2*padding)/canvas.height ]
    var aspect_2 = 0.5 * shape[0] / shape[1]
  ...
  return new TextSprites(gl, shader, buffer, vao, textures, axesStart, axesCount, labelOffset, size)
}